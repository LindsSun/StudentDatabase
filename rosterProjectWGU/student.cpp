...
//Constructors 
Student::Student()
	: studentId(""), firstName(""), lastName(""), email(""), age(-1), daysInCourse(new int[numDaysArraySize])
{
...
Student::Student(string studentId, string firstName, string lastName, string email, int age, int daysInCourse[], Degree degree)
	: studentId(studentId), firstName(firstName), lastName(lastName), email(email), age(age), daysInCourse(new int[numDaysArraySize])
{
	for (int i = 0; i < numDaysArraySize; ++i) {
...
void Student::setDaysInCourse(int daysInCourse[]) {
	this->daysInCourse = new int[numDaysArraySize];
